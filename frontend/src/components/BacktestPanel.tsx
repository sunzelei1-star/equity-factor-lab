...
const BacktestPanel = ({ config, onChange, onSubmit, loading }: Props) => {
  ...
  return (
    <section className="rounded-2xl border border-slate-800 bg-panel/90 p-5 shadow-panel">
      ...
      <button
        className="mt-6 w-full rounded-lg bg-cyan-500 px-4 py-2 font-semibold text-slate-900 transition hover:bg-cyan-400 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400"
        onClick={onSubmit}
        disabled={loading}
      >
